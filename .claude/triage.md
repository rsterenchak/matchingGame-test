...
- `drafted`:
  `{"state":"drafted","draft":"<full TODO.md entry>","file_paths":["src/...","..."],"thread":[{"role":"agent","text":"Drafted — ready to dispatch.","ts":"<now>"}]}`
...
- File paths full and repo-relative — `src/PlayPage.jsx`, never bare
  `PlayPage.jsx`. Source under `src/`, tests under `src/test/`.
- Do NOT invent an `<!-- id -->` marker — the app assigns it at inject time.
- Follow this repo's `CLAUDE.md` conventions (e.g. no new dependencies without an
  explicit instruction, plain CSS in `src/style.css`, no router or state-management
  library — read CLAUDE.md for the full list). Only mention a constraint that's
  actually relevant.
