...
- [x] **[MEDIUM]** Reposition home page Fight button above Safari chrome and move Nimbus GIF directly under the title
  - Type: bug
...
- [x] **[HIGH]** Fix Fight button and Goku gif rendering as vertical stack instead of layered — Completed: 2026-06-23
  - Type: bug
...
- [x] **[LOW]** Move Fight button down ~15% relative to the Goku gif — Completed: 2026-06-23
  - Type: feature
...
- [x] **[LOW]** Move nimbus cloud 10% left and 10% down on desktop only — Completed: 2026-06-23
  - Type: feature
...
- [x] **[LOW]** Lower background music volume to ~0.009 on both pages — Completed: 2026-06-23
  - Type: feature
...
- [x] **[MEDIUM]** Lower the default music volume below the current 0.009 — Completed: 2026-06-23
  - Type: feature
...
- [ ] **[HIGH]** Wire background music volume to the slider value and lower the hardcoded default
  - Type: bug
  - Description: The volume slider shows a low value but the music still plays loud because the `Handle*Audio` helpers set `audioRef.current.volume` to a hardcoded `0.07`, independent of the slider/default value — so the slider position and the actual Audio element volume are computed separately and never sync. Fix by (1) lifting the current volume value in `MainSection.jsx` (initialized from `localStorage` `matchingGame_volume`, falling back to `0.005`) and passing it as a prop to each `Handle*Audio` helper, and (2) replacing the hardcoded `0.07` with a volume-sync `useEffect` in each helper that sets `audioRef.current.volume` from that prop on mount and whenever it changes. Lower the effective default to `0.03` as the starting point. To set volume on the live instance reliably, move each helper's `new Audio(...)` construction into a `useRef` (per the CLAUDE.md audio note) so the volume-sync effect targets the same instance the play/pause logic uses; keep the existing `.play().then/.catch` autoplay guard and the `useEffect` cleanup-on-unmount pause. Acceptance: dragging the slider changes playback volume live on both the home and play tracks; a fresh session starts quiet; the slider's visual position always matches the audible level.
  - File: `src/MainSection.jsx`, `src/style.css`
  - Completed: YYYY-MM-DD (PR #<number>)
  <!-- id: 75bf4fc4-0df9-4b4d-a0c9-4afae391fa2e -->

- [x] **[HIGH]** Stop overlapping background-music instances so only the active track plays — Completed: 2026-06-23
  - Type: bug
